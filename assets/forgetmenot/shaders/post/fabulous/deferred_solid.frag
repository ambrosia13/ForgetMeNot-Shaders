...
uniform sampler2D u_previous_indirect;
...
layout(location = 1) out vec4 indirectResult;
...
void main() {
     ...
     if(isMetal) {
                    ...
                    if(min_depth == max_depth && max_depth < 1.0 && (rtaoRayPos + rayDir).z < 0.0) {
                         for(int j = 0; j < RTAO_STEPS; j++) {
                              ...
                              if(clamp01(rayScreenMarch) != rayScreenMarch) {
                                   ...
                                   float depthQuery = texelFetch(u_depth_mipmaps, ivec2(rayScreenMarch.xy * frxu_size), 0).r;

                                   if(rayScreenMarch.z > depthQuery && abs(linearizeDepth(rayScreenMarch.z) - linearizeDepth(depthQuery)) < 0.5) {
          ...
          //indirectResult = vec4(mix(lightmap, texture(u_previous_indirect, texcoord).rgb, 0.1), 1.0);
