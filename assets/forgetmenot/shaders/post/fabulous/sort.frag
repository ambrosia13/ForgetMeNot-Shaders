...
void main() {
    ...
    float min_depth = min(min(translucent_depth, particles_depth), main_depth);
        ...
        vec3 sky = frx_sampleTent(u_sky, texcoord * RESOLUTION_SCALE, 1.0 / frxu_size).rgb + frx_noise2d(texcoord) / 100.0;
    #else
        vec3 sky = texture(u_sky, texcoord * RESOLUTION_SCALE).rgb + frx_noise2d(texcoord) / 100.0;
    #endif

    vec3 cloudsColor = vec3(1.0);
    cloudsColor = mix(cloudsColor, vec3(0.2, 0.2, 0.3), getTimeOfDayFactors().z);
    cloudsColor = mix(cloudsColor, vec3(0.3, 0.3, 0.4), getTimeOfDayFactors().y);
    ...
    if(translucent_depth != max_depth) {
        ...
        if(all(lessThan(translucentf0.rgb - 0.04, vec3(0.001)))) {
            ...
            vec3 transReflectedView = reflect(viewSpacePos, translucentNormal + rand3D(texcoord) / 40.0);
            ...
            if(translucent_depth != max_depth) translucent_color = mix(translucent_color, vec4(transReflectedColor, 1.0), clamp01(vec4(transReflectance, frx_luminance(transReflectance))) * translucentData.a);
        }
    }
    vec3 solidf0 = solidData.ggg;
    vec3 solidNormal = texture(u_solid_normal, texcoord).rgb * 2.0 - 1.0;
    vec3 solidReflectedView = reflect(viewSpacePos, solidNormal + rand3D(texcoord) / 40.0);
    ...
    if(any(lessThan(abs(solidf0 - 1.0), vec3(0.01)))) { // metal
        ...
        main_color = mix(main_color, vec4(solidReflectedColor, 1.0), clamp01(vec4(metalColor, frx_luminance(metalColor))) * solidData.a);
    } else if(frx_rainGradient != 0.0 && dot(solidNormal, vec3(0.0, 1.0, 0.0)) > 0.7) {
        vec3 solidReflectance = getReflectance(solidf0, dot(viewDir, -solidNormal)) * frx_rainGradient;
        vec3 solidReflectedColor = mix(calculateSkyColor(normalize(solidReflectedView)), vec3(1.0), calculateBasicClouds(normalize(solidReflectedView)).x * cloudsColor);
    ...
    main_color.rgb = mix(
        ...
        (frx_worldIsOverworld == 1 ? mix(calculateSun(normalize(viewSpacePos)), vec3(0.0), clamp01(frx_rainGradient * 0.75 + frx_thunderGradient)) : vec3(0.0)) + 
        step(0.989, 1.0 - cellular(starCoord * 8.0).x) * (1.0 - getTimeOfDayFactors().x), floor(max_depth)
    );
