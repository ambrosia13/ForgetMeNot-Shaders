...
void frx_pipelineVertex() {
	...
	if (frx_modelOriginScreen) {
		...
		if((!frx_isGui || frx_isHand) && distance(frx_vertex.xyz + frx_cameraPos - frx_eyePos, vec3(0.0, -1.0, 0.0)) > 2.0) gl_Position.xy += getTaaOffset(frx_renderFrames) * (1.0 / vec2(frx_viewWidth, frx_viewHeight)) * gl_Position.w;
	#endif
}