...
vec3 atmosphericScattering(in vec3 viewSpacePos, in vec3 sunVector, in float factor, in float sunBrightness, float drawSun, float vlFactor) {
    ...
    if(frx_worldIsOverworld == 1 && drawSun > 0.0) {
        ...
        totalScatter += 4096.0 * drawSun * mix(80.0, 40.0, sqrt(LdotU)) * mie * opticalDepth * clamp01(diskVisibility);
    }
...
vec3 getSkyColorDetailed(in vec3 viewDir, in vec3 viewPos, in float drawSun) {
    ...
    if(frx_worldIsEnd == 1) {
        ...
        atmosphere += endSecondAtmosphere(secondViewDir, fNormalize(vec3(1.0, 0.1, 0.2)), 1.0, 1.0);
        if(secondViewDir.y < 0.0) {
            atmosphere += vec3(0.09, 0.12, 0.03) * smoothstep(-0.0, 1.0, dot(fNormalize(vec3(1.0, 0.6, 0.2)), secondViewDir));
