float waterHeightNoise(in vec2 uv) {
    ...
    vec2 coord = uv * 0.3;
        ...
        // waterHeight *= 0.5;
        float noiseA = snoise(rotate2D(coord, -PI / 12.0) * vec2(4.5, 1.0) + vec2(1.7, 1.2) * time) * 0.5 + 0.5;
        float noiseB = snoise(rotate2D(coord, PI / 12.0) * vec2(6.1, 1.2) - vec2(3.8, 10.2) * time) * 0.5 + 0.5;
